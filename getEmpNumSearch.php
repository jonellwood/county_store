...
if ($result->num_rows > 0) {
    ...
    echo "<button type='button' onclick='toggle()' class='btn btn-primary'>This order is for...</button>";
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<button type='button' class='invisi-button' onclick='setData(this.value)' value=" . $row['empNumber'] . ">" . $row['empName'] . "</button>";
    }
    echo "</div>";
    echo "</div>";
    echo "</div>";
