...
<head>
    ...
    function updateApproved(id) {
        ...
        alert("Item Changed Back to Ordered");
    ...
    function updateDenied(id) {
        ...
        ajax.onreadystatechange = function() {
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                alert("Item Has Been Changed Back to Denied Status");
                window.location.href = '../pages/received.php';
...
WHERE status = 'Received'";
    ...
    OR dr.dep_assist = $empNumber AND status = 'Received'";
}
                ...
                <div class="col-12">
                    <div class="card mb-4">
                        <div class="card-header pb-0">
                            <h6>You have Received <font color="green"> <?php echo $ORDERS ?> Total Orders from Vendor.
                                </font>
                            </h6>
                            <h6>These Orders combine for a total of <font color="gray"> <?php echo $INVENTORY ?> items
                                    in inventory</font>
                            </h6>
                            <h6>These Items You Now Have on Hand have a grand total of: <font color="blue">
                        ...
                        <div class="card-body px-0 pt-0 pb-2">
                            <div class="table-responsive p-0">
                                <table class="table align-items-center mb-0">
                                    <thead>
                                        <tr>
                                            <th
                                                ...
                                                Item Received for Employee</th>
                                            <th
                                                class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                                Description of Item</th>
                                            <th
                                                class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                                Number of Items Received In this Order</th>
                                            <th
                                                class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                                Total Cost of Received Order</th>
                                            <th
                                                class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                                Date Requested</th>
                                    ...
                                    <?php
                                    if ($ROLE === "Administrator") {
                                        $sql = "SELECT ord.status, ord.size_id, ord.color_id, ord.comment, ord.emp_id, ord.created, ord.quantity, ord.order_details_id, ord.line_item_total, ord.product_name, ord.product_price, ord.product_code, ord.vendor,
                                    ord.submitted_by, d.dep_name AS dept_name, er.empName as ordered_for, er.email, ord.logo
                                    FROM ord_ref ord 
                                    ...
                                    WHERE status = 'Received'";
                                        ...
                                        $sql = "SELECT ord.status, ord.size_id, ord.color_id, ord.emp_id, ord.comment, ord.created, ord.quantity, ord.order_details_id, ord.line_item_total, ord.product_name, ord.product_price, ord.product_code, ord.vendor,
                                      ord.submitted_by, d.dep_name AS dept_name, er.empName as ordered_for, er.email, ord.logo
                                      FROM ord_ref ord 
                                      JOIN emp_ref er ON ord.emp_id = er.empNumber 
                                      JOIN departments dr on ord.department = dr.dep_num
                                      JOIN (SELECT * from departments) d on d.dep_num = er.deptNumber
                                      WHERE status = 'Received' AND dr.dep_head = $empNumber
                                      OR dr.dep_assist = $empNumber AND status = 'Received'";
                                    }
                                    $c = 1;
                                    $result = mysqli_query($conn, $sql);
                                    if (mysqli_num_rows($result) > 0) {
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            echo "<h5 class='modal-title' id='exampleModalLabel'>You Now Have this Item On Hand Ready to give to <font color = '#58c9e8'> $EMPNAME</font></h5>";
                                            ...
                                            echo "</tr>";
                                            echo "<tr>";
                                            echo "<td>Total Cost for All Items</td>";
