        ...
        <div class="div2">
            ...
            <div id="requestEdit" popover=manual>
                ...
                <div class="edit-holder">
                    ...
                    <button form="edit" type="submit" class="submit-btn">Update</button>
                </div>
        ...
        fetch('check-size-and-color.php?c_id=' + current + '&s_id=' + selected + '&c_size=' + size + '&c_color=' +
            ...
            .then(data => {
                ...
                if (data.status == 200) {
                    ...
                    html +=
                        `<button onclick="setPriceDisplayInForm('${data.selected_price}')" popovertarget="mismatch-alert" popovertargetaction="hide">Change Anyway</button>`
                    html +=
                        `<button popovertarget="mismatch-alert" popovertargetaction="hide">Do Not Change</button>`
                    html += `</div>`
                    ...
                    if (data.status == 404) {
                        ...
                        html += `<div class='button-holder'>`;
                        html +=
                            `<button onclick="resetValues('${data.selected_price}', ${selected}, '${removeSingleQuotes(data.s_name)}')" popovertarget="mismatch-alert" popovertargetaction="hide">Change Anyway</button>`
                        html +=
                            `<button onclick="setCurrentProductCode(${current})" popovertarget="mismatch-alert" popovertargetaction="hide">Do Not Change</button>`
                        html += `</div>`
                        document.getElementById('alert-message').innerHTML = html;
