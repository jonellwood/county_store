...
<head>
    ...
    <title>Department ORDERED Reports</title>
</head>
...
if ($ROLE === "Administrator") {
    ...
    WHERE status = 'Ordered' AND dep_num = $DEPTID";
} else {
    $sql = "SELECT SUM(quantity) as TOTALQNT, dr.dep_name
    FROM uniform_orders.ord_ref o
    JOIN dep_ref dr on o.department = dr.dep_num
WHERE (status = 'Ordered' AND dr.dep_head = $DEPT OR dr.dep_assist = $DEP and status = 'Ordered')";
}
        ...
        <div id="wrapper">
            <div id="content">
                ...
                if ($ROLE === "Administrator") {
                    ...
                    JOIN dep_ref dr on o.department = dr.dep_num
                    WHERE status = 'Ordered' AND e.deptNumber = $DEPTID GROUP BY product_code, size_name, color_id ORDER BY SUM(o.quantity) DESC";
                } else {
                    $sql = "SELECT o.product_code, o.size_name, o.color_id, SUM(o.quantity) as TALLY, o.product_price
                    FROM uniform_orders.ord_ref o
                    JOIN emp_ref e on o.emp_id = e.empNumber
                    JOIN dep_ref dr on o.department = dr.dep_num
                    WHERE status = 'Ordered' AND e.deptNumber = $DEPTID AND dr.dep_head = $DEPT OR dr.dep_assist = $DEPT and status = 'Ordered' GROUP BY product_code, size_name, color_id ORDER BY SUM(o.quantity) DESC";
                }
                $result = mysqli_query($conn, $sql);
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                ...
                if ($ROLE === "Administrator") {
                    ...
                    join emp_ref e on o.emp_id = e.empNumber
                    JOIN dep_ref dr on o.department = dr.dep_num
                    WHERE status = 'Ordered' and e.deptNumber = $DEPTID";
                } else {
                    $sql = "SELECT o.size_name, o.status, o.color_id, e.empName, o.emp_id, o.product_name, o.product_price, o.created, o.requested_by_name,
                    o.quantity, o.product_name, e.email, e.deptName, e.deptNumber, e.empNumber, o.line_item_total, o.order_id, o.order_details_id, o.customer_id, o.created, o.logo, o.product_code
                    FROM uniform_orders.ord_ref o
                    join emp_ref e on o.emp_id = e.empNumber
                    JOIN dep_ref dr on o.department = dr.dep_num
                    WHERE (status = 'Ordered' AND dr.dep_head = $DEPT OR dr.dep_assist = $DEPT and status = 'Ordered')";
                }
                $result = mysqli_query($conn, $sql);
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
