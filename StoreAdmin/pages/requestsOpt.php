...
<head>
    ...
    $sql = "SELECT 
        (SELECT COUNT(order_details_id) FROM count_ref WHERE status = 'Pending') AS PENDINGCOUNT,
        (SELECT COUNT(distinct(emp_id)) FROM count_ref WHERE status = 'Pending') AS EMPLOYEES,
        (SELECT COUNT(distinct(department)) FROM count_ref WHERE status = 'Pending') AS DEPTT
        FROM count_ref cr 
    ...
    if ($ROLE === "Administrator") {
        ...
        $sql .= "AND dr.dep_head = $empNumber OR dr.dep_assist = $empNumber";
    }
                            ...
                            <div class="table-responsive p-0">
                                <table class="table align-items-center mb-0">
                                    ...
                                    $sql = "SELECT distinct(ord.order_details_id), ord.order_id, ord.emp_id, ord.created, ord.quantity, ord.line_item_total, ord.product_name, 
                                            ord.submitted_by, CONCAT(c.first_name, ' ', c.last_name) as ordered_for, c.email, ord.status 
                                            FROM ord_ref ord 
                                            JOIN customers c on ord.emp_id = c.emp_id 
