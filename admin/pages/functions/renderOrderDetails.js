...
function renderOrderDetails(data) {
	...
	html += `
        <div class='main-order-info-holder w-100'>
            <div class='stats-key-holder'>   
                <span class='stat-item'><p class='status-pills Pending'>P</p> = Pending</span>
                <span class='stat-item '><p class='status-pills Approved'>A</p> = Approved</span>
                <span class='stat-item '><p class='status-pills Denied'>D</p> = Denied</span>
                <span class='stat-item '><p class='status-pills Ordered'>O</p> = Ordered</span>
                <span class='stat-item '><p class='status-pills Received'>R</p> = Received</span>
	...
	if (allOrdered) {
		html += `<span class='table-title'>Order Details for ${data[0].rf_first_name} ${data[0].rf_last_name} 
                    <button class='btn btn-receive' value='${data[0].order_id}' popovertarget='receive-whole-order-confirm' popoveraction='show' onclick="createWholeOrderReceivePopover(${data[0].order_id})"> Receive Whole Order </button>`;
	} else if (allPending) {
		html += `<span class='table-title'>Order Details for ${data[0].rf_first_name} ${data[0].rf_last_name}<button class='btn btn-approve' value='${data[0].order_id}' popovertarget='action-jackson' onclick="createWholeOrderActionPopover(${data[0].order_id})"> Approve or Deny Order </button>`;
		if (hasVendorID3) {
	...
	for (var j = 0; j < data.length; j++) {
		...
		html += `<tr class='${data[j].status}' onclick="setLineItemSession(${
			data[j].order_details_id
		}, '${data[j].status}', ${data[j].department})"'>
                            <td class="${data[j].status}"> 
                                <p class="status-pills">${firstChar(
																	data[j].status
																)} </p>
                            </td>
                            <td>${data[j].quantity ? data[j].quantity : ''}</td>
                            <td colspan=3>${
															data[j].product_code ? data[j].product_code : ''
														}</td>
                            <td>${
															data[j].size_name ? data[j].size_name : ''
														}</td>
                            <td>${
															data[j].color_name ? data[j].color_name : ''
														}</td>
                            <td>${
															data[j].line_item_total
																? money_format(data[j].line_item_total)
																: ''
														}</td>
                            <td><img src=../../${
															data[j].logo ? data[j].logo : ''
														} alt=${
			data[j].product_code ? data[j].product_code : ''
		} class='small-logo-img'/></td>
                            <td>${
															data[j].dept_patch_place
																? data[j].dept_patch_place
																: 'N/A'
														}</td>
		...
		html += `<td id='prod_img_${j}'><img class='img prod_img' src=../../product-images/${
			data[j].color_name ? formatColorValueForUrl(data[j].color_name) : ''
		}_${
			data[j].product_code ? formatValueForUrl(data[j].product_code) : ''
		}.jpg alt='..' width='50px'/></td></tr>`;
	}
	html += `</tbody>
            </table>
	...
	var orderTotal = `<p class='receipt'>Request Total: ${money_format(
		totalCost
	)} </p>`;
	...
	const renderOrderSummary = (orderTotal, itemTotal) => {
		...
		summaryContainer.innerHTML = `<p class='receipt'>Request Total: ${money_format(
			orderTotal
		)} </p>`;
	...
	const renderPopover = (orderTotal, itemTotal) => {
		...
		popoverValues.innerHTML = `
            <p>Confirm your decision for these <b>${itemTotal}</b> items - for a cost of  <b>$${money_format(
			orderTotal
		)}</b></p>
