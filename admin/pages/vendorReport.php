...
// Last Modified: 2025/03/18 09:08:59
session_start();
        ...
        .then((data) => {
            ...
            for (var i = 0; i < splitObjs.length; i++) {
                ...
                for (var j = 0; j < splitObjs[i].length; j++) {
                    const comments = splitObjs[i][j].comments ?
                        splitObjs[i][j].comments.split(' || ') : [];
                    const commentSubmitted = splitObjs[i][j].comment_submitted.split(' || ') ?
                        splitObjs[i][j].comment_submitted.split(' || ') : []
                    const comment_submitter = splitObjs[i][j].comment_sub_name.split(' || ') ?
                        splitObjs[i][j].comment_sub_name.split(' || ') : []
                    html += `<tr>    
                                ...
                                <td>${splitObjs[i][j].rf_first_name} ${splitObjs[i][j].rf_last_name} </br>
                                    ${splitObjs[i][j].comment}</br>`

                    if (comments.length > 0) {
                        html += `<ul class='comments-table'>`;

                        comments.forEach((comment, index) => {
                            const submitted = commentSubmitted[index] || 'N/A';
                            html +=
                                `<l1>${comment} - ${comment_submitter[index]} (${formatDate(submitted)})</li></br>`;
                        });
                        html += `</ul>`
                    } else {
                        html += `No comments submitted`;
                    }
