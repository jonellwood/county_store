...
$sql = "UPDATE order_details SET status = 'Approved', status_id = 1 where order_id = '$order_id'";
$stmt = $conn->prepare($sql);
$updated = $stmt->execute();

if ($updated) {
    $line_items_to_approve = array();
    ...
    if ($orderIdList->num_rows > 0) {
        while ($row = $orderIdList->fetch_assoc()) {
            array_push($line_items_to_approve, $row);
        }
    }
    foreach ($line_items_to_approve as $item) {
