    ...
    async function editProduct(id) {
        await fetch('load-new-single-product-colors.php?id=' + id)
            ...
            .then((data) => {
                ...
                var html = '';
                html +=
                    `<h2>Edit color details for ${data[2].current_product[0].name} - ${data[2].current_product[0].code} </h2>`
                html += `<form action="edit-new-product-colors-db.php" method="POST">`
                html +=
                    `<input type='hidden' value='${data[2].current_product[0].product_id}' name='productId' />`;
                html += `<fieldset id='color_options'>`;
                html +=
                    `<span><b>Color Options</b> - Currently assigned values are already checked. Check or uncheck to assign or remove color options for this product. <mark>ONLY ITEMS THAT ARE CHECKED WILL BE AVAILABLE TO USERS</mark></span>`
                html += `
                        <ul class='color-list'>`
                for (var j = 0; j < data[1].all_colors.length; j++) {
                    ...
                    var fallbackThirdColor = colors[j].t_hex !== null ? colors[j].t_hex : (colors[j].s_hex !==
                        null ? colors[j].s_hex : colors[j].p_hex);
                    var gradientString =
                        `linear-gradient(to right, ${colors[j].p_hex}, ${fallbackSecondColor}, ${fallbackThirdColor})`;
