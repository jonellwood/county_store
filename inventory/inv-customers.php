    ...
    <script>
        ...
        async function getEmpInv(employeeID) {
            await fetch('./get-emp-inventory.php?emp_id=' + employeeID)
                ...
                .then(data => {
                    console.log('Here comes the 43');
                    console.table(data);
                    var background = document.getElementById('main-table');
                    background.classList.add('muted');
                    var html = "<td><table class='emp-inv-table'>"
                    html +=
                        "<tr><th>Order ID</th><th>Product Code</th><th>Size</th><th>Color</th><th>Price</th><th>Logo</th><th></th></tr>"
                    for (var i = 0; i < data.length; i++) {


                        html += "<tr><td>"
                        html += data[i].order_details_id;
                        html += "</td>";
                        html += "<td>";
                        ...
                        html + -"</td>";
                        html += "<td>";
                        html += data[i].size_name;
                        html + -"</td>";
                        html += "<td>";
                        html += "<img src=../" + data[i].logo + " alt='logo' width=50px />"
                        html + -"</td>";
                        html += "<td>";
                    }
            ...
            <h5><em>TODO:</em>
                <ul>
                    <li class='lilList'>Make secondary query a foreach loop to populate all items in inventory for that
                        employee. DONE BUT STILL UGLY AND SLOW</li>
            ...
            <div class="container">
                <div class="make-dim ">
                    ...
                    <table class='main-table' id='main-table'>
                        ...
                        <tr>
                            <!-- The button generated here gets the inventory for the employee and populates a sub-table with the list using the getEmpInv function. TODO move to off canvas element -->
                            <?php foreach ($empList as $emp) {
...
<style>
    ...
    .emp-inv-table {
        ...
        z-index: 5;
        visibility: visible;
        border: 15px solid #789b48;
        box-shadow: 0px 0px 27px 45px rgba(120, 155, 72, 0.54);
    }
