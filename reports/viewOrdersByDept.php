...
<head>
    ...
    async function getDepartments() {
        await fetch('getDepartments.php')
    ...
    async function getOrdersByDepartment(department, status) {
        console.log(status);
        await fetch('getOrdersByDept.php?department=' + department + '&status=' + status)
            .then((response) => response.json())
            .then((data) => {
                console.table(data);
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    html += '<td class="img"><img src="../' + data[i].logo + '" alt="nogo"/></td>';
...
<?php include "../StoreAdmin/pages/topNav.php" ?>
        ...
        <form id="orderForm" onsubmit="submitForm(event)" class="orderForm">
            <div class="drop-down-holder">
                ...
                <select id="status" name="status" required>
                    <option
                        value="(status = 'Pending' OR status = 'Approved' OR status = 'Ordered' OR status = 'Received' OR status = 'Denied')">
                        All</option>
