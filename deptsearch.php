...
o.quantity, o.product_name, e.email, e.deptName, e.empNumber, o.order_id, o.order_details_id, o.customer_id, o.created, e.deptNumber
...
join emp_ref e on o.emp_id = e.empNumber
JOIN dep_ref dr on o.department = dr.dep_num
WHERE (e.deptNumber = $empidd AND dr.dep_head = $DEPT)";
}
...
if ($ROLE === "Administrator") {
    ...
    SUM(o.line_item_total) as APPSUM, COUNT(o.order_details_id) as APPCOUNT
    FROM uniform_orders.ord_ref o
    join emp_ref e on o.emp_id = e.empNumber
    JOIN dep_ref dr on o.department = dr.dep_num
WHERE status = 'Approved' AND e.deptNumber  = $empidd";
    ...
    join emp_ref e on o.emp_id = e.empNumber
    JOIN dep_ref dr on o.department = dr.dep_num
WHERE (e.deptNumber = $empidd AND dr.dep_head = $DEPT) AND status = 'Approved'";
}
$APPROVEDREQ = 0;
$APPROVEDSUM = 0;
$result = mysqli_query($conn, $sql);
...
WHERE status = 'Denied' AND e.deptNumber  = $empidd";
    ...
    join emp_ref e on o.emp_id = e.empNumber
    JOIN dep_ref dr on o.department = dr.dep_num
WHERE (e.deptNumber = $empidd AND dr.dep_head = $DEPT) AND status = 'Denied'";
}
$DENIEDREQ = 0;
$DENIEDSUM = 0;
$result = mysqli_query($conn, $sql);
...
WHERE status = 'Pending' AND e.deptNumber  = $empidd";
    ...
    join emp_ref e on o.emp_id = e.empNumber
    JOIN dep_ref dr on o.department = dr.dep_num
WHERE (e.deptNumber = $empidd AND dr.dep_head = $DEPT) AND status = 'Pending'";
}
$PENDINGREQ = 0;
$PENDINGSUM = 0;
$result = mysqli_query($conn, $sql);
...
WHERE status = 'Ordered' AND e.deptNumber  = $empidd";
    ...
    join emp_ref e on o.emp_id = e.empNumber
    JOIN dep_ref dr on o.department = dr.dep_num
WHERE (e.deptNumber = $empidd AND dr.dep_head = $DEPT) AND status = 'Ordered'";
}
$ORDEREDREQ = 0;
$ORDEREDSUM = 0;
$result = mysqli_query($conn, $sql);
...
WHERE status = 'Received' AND e.deptNumber  = $empidd";
    ...
    join emp_ref e on o.emp_id = e.empNumber
    JOIN dep_ref dr on o.department = dr.dep_num
WHERE (e.deptNumber = $empidd AND dr.dep_head = $DEPT) AND status = 'Received'";
}
$RECEIVEDREQ = 0;
$RECEIVEDSUM = 0;
$result = mysqli_query($conn, $sql);
...
<body class="g-sidenav-show   bg-gray-100" onload="setDark()">
    ...
    <main class="main-content position-relative border-radius-lg ">
        ...
        <div class="container-fluid py-4">
            <div class="row">
                <div class="col-12">
                    <div class="card mb-4">
                        <a href="deptPrint.php?demptid=<?php echo $empidd ?>" class="btn btn-primary" role="button">Print or Save this as PDF to submit to Vendor</a>
                        <div class="card-header pb-0">
                            <h6>There has been a total of <strong>
                                    <font color="black"> <?php echo $REQUESTS ?></font>
                        ...
                        <div class="card-body px-0 pt-0 pb-2">
                            <div class="table-responsive p-0">
                                <table class="table align-items-center mb-0">
                                    ...
                                    <?php
                                    if ($ROLE === "Administrator") {
                                        $sql = "SELECT ord.status, ord.size_id, ord.color_id, ord.size_name, ord.emp_id, ord.comment, ord.created, ord.quantity, ord.order_details_id, ord.line_item_total, ord.product_name, ord.product_price, ord.product_code, ord.vendor,
                                        ...
                                        JOIN dep_ref dr on ord.department = dr.dep_num
                                        JOIN (SELECT * from departments) d on d.dep_num = er.deptNumber
                                        WHERE d.dep_num = $empidd AND dr.dep_head = $DEPT";
                                    }
                                    $c = 1;
                                    $result = mysqli_query($conn, $sql);
                                    if (mysqli_num_rows($result) > 0) {
