    ...
    function fetchProductData(id) {
        fetch('fetchProductDetails.php?id=' + <?php echo $product_id ?>)
            ...
            .then((data) => {
                ...
                var html = '';
                html +=
                    `<form name='option' method='post' id='options' action='cartAction.php' class='options-select-holder'>`;
                html +=
                    `<input type="hidden" name="product_id" id="product_id" value=${data['product_data'][0].product_id} />`;
                ...
                html +=
                    `<input type="hidden" name="color_name" id="color_name" value="${data['color_data'][0].color}" />`
                html += `<input type="hidden" name="size_id" id="size_id" value=${data['price_data'][0].size_id} />`
                html +=
                    `<input type="hidden" name="size_name" id="size_name" value="${data['price_data'][0].size_name}" />`
                ...
                <div id='color-picker-holder hidden'>
                    <legend class="hidden">Pick a Color</legend>
                    <label for="color_id" class="legend hidden"></label>
                    <select title="color_id" name="color_id" id="color_id" onchange="updateProductImage(this.value)" class="hidden">
            `;
                for (var i = 0; i < data['color_data'].length; i++) {
                    html +=
                        `<option id="${data['color_data'][i].color}" value="${data['color_data'][i].color}" data-hex="${data['color_data'][i].p_hex}" data-colorname="${data['color_data'][i].color}">${data['color_data'][i].color}  </option>`;
                }
                html += `
                    </select>
                </div>
            `;
                html += `
                    <div id='size-picker-holder hidden'>
                        <legend class="hidden">Pick a Size</legend>
                        <div class="size-price-picker-holder hidden">
                        `;

                for (var j = 0; j < data['price_data'].length; j++) {
                    html += `<label for='${data['price_data'][j].price_id}'>`;
                    html +=
                        `<input type='radio' id=${data['price_data'][j].price_id} value=${data['price_data'][j].price_id} name='size-price-id' data-priceval=${data['price_data'][j].price} data-sizeid=${data['price_data'][j].size_id} data-sizename="${data['price_data'][j].size_name}" onchange='updateHiddenPriceInput(this)'`;
                    if (j === 0) {
                        html += `checked`;
                    }
                    html += ` />
                ...
                html += `
                        <div id="logo-info-holder" class="logo-info-holder hidden">
                ...
                html += `
                        ...
                        <div class="dept-name-patch-holder hidden">
...
<body>
    ...
    <div class="container" id="container">
        ...
        <div class="another-container" id="another-container">
            ...
            <span class='product-image-holder card' id='product-image-holder' view-transition-group="image-transition">

            </span>

            <div id='new-options-form' class="hidden-form hidden"></div>
        ...
        <div class=" button-holder">
            <a href=<?php echo $_SESSION['GOBACK'] ?>><button class="button btn btn-secondary" type="button">← Continue
                    Shopping </button></a>
            ...
            <button type="submit" form="options" class="button btn btn-primary custom-btn"><span> Add to Cart
                    ➕</span></button>
        </div>
