...
$sql = "SELECT 
  ...
  SUM(IF(status = 'Pending', line_item_total, 0.00)) as total_submitted,
  SUM(IF(status = 'Approved', line_item_total, 0.00)) as total_approved,
   SUM(IF(status = 'Ordered', line_item_total, 0.00)) as total_ordered,
  SUM(IF(status = 'Received', line_item_total, 0.00)) as total_completed,
  SUM(status = 'Submitted') as count_submitted,
  SUM(status = 'Approved') as count_approved,
  SUM(status = 'Denied') as count_denied,
  SUM(status = 'Received') as count_received
FROM 
                ...
                <tr>
                    <td>
                        <table role='presentation' style='width:100%;border-collapse:collapse;border:0;border-spacing:0; padding:'>
                            <tr>
                                <th style='text-align: center; width:25%;'>Pending Approval</th>
                                <th style='text-align: center; width:25%;'>Approved</th>
                                <th style='text-align: center; width:25%;'>On Order</th>
